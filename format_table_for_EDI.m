...
for i = 1:height(EDI_table); 
    ...
    EDI_table.syn_cells_per_ml(i) = sum(C.class==2)./gated_table.Vol_ml(gind); 
    EDI_table.syn_biovolume_concentration(i) = nansum(volume(C.class==2))./gated_table.Vol_ml(gind); 
    EDI_table.syn_carbon_concentration(i) = nansum(carbon(C.class==2))./gated_table.Vol_ml(gind)./1000; %divide by 1000 to get micrograms per Liter
    EDI_table.syn_volume_analyzed_ml(i) = gated_table.Vol_ml(gind); 
    ...
    EDI_table.redeuk_leq_2um_cells_per_ml(i) = length(bin_particle_ind)./gated_table.Vol_ml(gind); %counts over volume
    EDI_table.redeuk_leq_2um_biovolume_concentration(i) = nansum(volume(bin_particle_ind))./gated_table.Vol_ml(gind); 
    EDI_table.redeuk_leq_2um_carbon_concentration(i) = nansum(carbon(bin_particle_ind))./gated_table.Vol_ml(gind)./1000; %divide by 1000 to get micrograms per Liter

    % <= 3
    bin_particle_ind = find(diam'<=3 & C.class==1)';
    EDI_table.redeuk_leq_3um_cells_per_ml(i) = length(bin_particle_ind)./gated_table.Vol_ml(gind); %counts over volume
    EDI_table.redeuk_leq_3um_biovolume_concentration(i) = nansum(volume(bin_particle_ind))./gated_table.Vol_ml(gind); 
    EDI_table.redeuk_leq_3um_carbon_concentration(i) = nansum(carbon(bin_particle_ind))./gated_table.Vol_ml(gind)./1000; %divide by 1000 to get micrograms per Liter

    % <= 5
    bin_particle_ind = find(diam'<=5 & C.class==1)';
    EDI_table.redeuk_leq_5um_cells_per_ml(i) = length(bin_particle_ind)./gated_table.Vol_ml(gind); %counts over volume
    EDI_table.redeuk_leq_5um_biovolume_concentration(i) = nansum(volume(bin_particle_ind))./gated_table.Vol_ml(gind); 
    EDI_table.redeuk_leq_5um_carbon_concentration(i) = nansum(carbon(bin_particle_ind))./gated_table.Vol_ml(gind)./1000; %divide by 1000 to get micrograms per Liter

    % <= 10
    bin_particle_ind = find(diam'<=10 & C.class==1)';
    EDI_table.redeuk_leq_10um_cells_per_ml(i) = length(bin_particle_ind)./gated_table.Vol_ml(gind); %counts over volume
    EDI_table.redeuk_leq_10um_biovolume_concentration(i) = nansum(volume(bin_particle_ind))./gated_table.Vol_ml(gind); 
    EDI_table.redeuk_leq_10um_carbon_concentration(i) = nansum(carbon(bin_particle_ind))./gated_table.Vol_ml(gind)./1000; %divide by 1000 to get micrograms per Liter

    % <= 20
    bin_particle_ind = find(diam'<=20 & C.class==1)';
    EDI_table.redeuk_leq_20um_cells_per_ml(i) = length(bin_particle_ind)./gated_table.Vol_ml(gind); %counts over volume
    EDI_table.redeuk_leq_20um_biovolume_concentration(i) = nansum(volume(bin_particle_ind))./gated_table.Vol_ml(gind); 
    EDI_table.redeuk_leq_20um_carbon_concentration(i) = nansum(carbon(bin_particle_ind))./gated_table.Vol_ml(gind)./1000; %divide by 1000 to get micrograms per Liter


    EDI_table.redeuk_volume_analyzed_ml(i) = gated_table.Vol_ml(gind); 
    ...
    % <= 5
        ...
        EDI_table.redeuk_filename{i} = 'NA';
    ...
    EDI_table.hetprok_cells_per_ml(i) = CNTable.hetbac_per_ml(i); % Do NOT go back to class file, since we had to account for time gate 
    %EDI_table.hetprok_biovolume_concentration(i) = nansum(volume(C.class==3))./gated_table.Vol_ml(gind); 
    %EDI_table.hetprok_carbon_concentration(i) = nansum(carbon(C.class==3))./gated_table.Vol_ml(gind)./1000; %divide by 1000 to get micrograms per Liter
    EDI_table.hetprok_volume_analyzed_ml(i) = gated_table.Vol_ml(gind); 
    EDI_table.hetprok_filename(i) = CNTable.BacteriaFile(i); 
        ...
        EDI_table.hetprok_filename{i} = 'NA';
    end
