...
%modifying (3/7/23) so that it looks for hierarchical gates from attune
%software, described as "parent" gates in the aws files. 
...
gate_assignment_noparents = zeros(length(gates_to_do), length(fcsdat)); 
...
for g = 1:length(gates_to_do)
    ...
    gate_assignment_noparents(g, in_gate) = 1; 

end


%% now make sure each gate assingment considers the parents 

gate_assignment = zeros(length(gates_to_do), length(fcsdat)); 
gates_left = 1:length(gates_to_do);

num_parents = nan(1,length(gates_to_do)); %number of hierarchical parents for each gate


no_parents = find(cellfun(@isempty, parent_logic)); %first do gates without any 
num_parents(no_parents) = 0; 

gate_assignment(no_parents, :) = gate_assignment_noparents(no_parents, :); 
gates_left(no_parents) = NaN;

num_parents_rn = 1; %which generation are we working on right now?
while any(~isnan(gates_left)) %go until all gates have been checked. 
    
    past_parents = find(num_parents == num_parents_rn -1); %go through gates at previous hierarchichal level 
    for i = past_parents
        rn_parent1 = strcmp(gate_names(i), parent_logic); %find which gates in this level depend on past parent i 
        num_parents(rn_parent1) = num_parents_rn; %save number of parents for these gates
        gate_assignment(rn_parent1, :) = gate_assignment_noparents(rn_parent1, :) & gate_assignment(i,:); %add original gate logic to recently derived parent logic 
        gates_left(rn_parent1) = NaN; %remove this gate from to do list
    end
    num_parents_rn = num_parents_rn +1; %go on to next hierarchical level 
end
